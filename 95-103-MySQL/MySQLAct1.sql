...
SELECT * FROM empleados WHERE comision_emp <= (sal_emp * .3);
...
SELECT MAX(sal_emp) AS Salario_mas_alto, MIN(sal_emp) AS Salario_mas_bajo, (MAX(sal_emp) - MIN(sal_emp)) AS Diferencia_entre_ellos FROM empleados;
...
SELECT COUNT(id_emp) FROM empleados GROUP BY id_depto HAVING COUNT(id_emp) > 3;
-- 26. Hallar los departamentos que no tienen empleados
SELECT COUNT(id_emp) FROM empleados GROUP BY id_depto HAVING COUNT(id_emp) = 0;
